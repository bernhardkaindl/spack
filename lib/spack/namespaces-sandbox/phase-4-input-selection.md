...
network policy are later work items.
...
- The existing `mount_setattr` requirement implies Linux 5.12 or later and
  glibc 2.36 or later for the libc wrapper. `open_tree`, `move_mount`,
  `fsopen`, and `fsmount` need Linux 5.2 and the same glibc, so they add no
  new platform floor.
...
- [ ] Inventory the whole-child writable set: stage, install prefix,
  ...
  site-packages, including a virtual environment).
...
- [ ] Resolve the mount-point conflict before selecting stage and prefix
  mounts. Proposed: the supervisor performs prefix pivot, restore, and
  failed-prefix removal outside the namespace. Trusted pre-thread setup
  creates the stage and an empty prefix, and the child writes only below
  those identity mounts. Reject a writable store or stage root, because it
  exposes concurrent builds.

### Mount-plan scratch

- [ ] Replace durable scratch with detached mounts: clone preserved sources
  with `open_tree(OPEN_TREE_CLONE)`, create mask sources with
  `fsopen`/`fsmount`, and attach them with `move_mount`. This removes the
  scratch placement problem and anchors sources by descriptor, closing the
  substitution hardening item in `refactor-review.md`. This proposal
  supersedes the source item's durable-scratch requirement once accepted. If
  it is rejected, allocate private scratch outside all hidden and replacement
  roots, with supervisor-owned cleanup.
...
- [ ] B2, `sandbox: preserve namespace sources with detached mounts`,
  including the capability probe and fallback decision.
- [ ] B3, `sandbox: prove a read-only view with explicit writable mounts`.
- [ ] C1, `installer: move prefix pivoting outside the namespace child`.
