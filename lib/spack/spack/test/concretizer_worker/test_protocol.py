...
def test_request_accepts_deprecation_metadata_outside_reuse_snapshot():
    request = concretizer_worker.create_request(
        [Spec("pkg-a")], local_deprecated_for={"deprecated-hash": "replacement-hash"}
    )

    assert concretizer_worker.validate_request(request).local_deprecated_for == {
        "deprecated-hash": "replacement-hash"
    }
...
@pytest.mark.parametrize("kwargs", [{"local_external_origin_hashes": ["unknown-hash"]}])
def test_request_rejects_unmatched_local_external_metadata(kwargs):
    request = concretizer_worker.create_request([Spec("pkg-a")])
