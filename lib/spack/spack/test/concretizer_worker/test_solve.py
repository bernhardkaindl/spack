...
def test_worker_matches_direct_multi_root_together_solve(mock_packages):
    ...
    direct = Solver(context=spack.context.default()).solve(abstract).specs or []
...
def test_compiler_preflight_includes_configured_and_installed(mock_packages, monkeypatch):
    ...
    class Detector:
        def __init__(self, compiler, *, repo, cache):
    ...
    monkeypatch.setattr(compiler_config, "supported_compilers", lambda *, repo: ["gcc", "llvm"])
    ...
    worker_solve._preflight_compiler_properties(
        spack.context.default(), [configured], [configured, installed, not_a_compiler]
    )
...
def test_worker_static_analysis_uses_frozen_buildcache_snapshot(
    mock_packages, mutable_config, monkeypatch
):
    import spack.solver.reuse
    ...
    response = concretizer_worker.solve_in_worker([Spec("pkg-a foobar=baz")])
