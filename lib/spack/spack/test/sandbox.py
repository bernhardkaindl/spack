...
def test_sandbox_policy_is_loaded_from_yaml():
    policy = spack.installer.build._load_sandbox_policy(spack.installer.build.SANDBOX_POLICY_PATH)

    assert tuple(policy["commands"]) == spack.installer.build.SANDBOX_COMMANDS
    assert (
        tuple(policy["host_runtime_read_paths"]) == spack.installer.build.HOST_RUNTIME_READ_PATHS
    )
    assert "tee" in policy["coreutils_file_programs"]
    assert "sha384sum" in policy["build_utilities_programs"]
...
def _system_compiler_spec(name, version, compiler_path):
    ...
    edge = SimpleNamespace(spec=compiler, virtuals=("cxx",))
    root = SimpleNamespace(edges_to_dependencies=lambda: [edge])
    return SimpleNamespace(traverse=lambda: [root])
