...
def _load_sandbox_policy(path: str = SANDBOX_POLICY_PATH) -> dict:
    """Load and validate the shipped build sandbox compatibility policy."""
    try:
        with open(path, encoding="utf-8") as stream:
            policy = syaml.load(stream)
    except (OSError, syaml.SpackYAMLError) as error:
        raise spack.error.InstallError(f"Cannot load sandbox policy {path}: {error}") from error

    if not isinstance(policy, dict) or policy.get("version") != 1:
        raise spack.error.InstallError(f"Invalid sandbox policy version in {path}")
    list_keys = (
        "commands",
    ...
    for key in list_keys:
        ...
        if not isinstance(value, list) or not all(isinstance(entry, str) for entry in value):
            raise spack.error.InstallError(f"Invalid sandbox policy list {key} in {path}")
    aliases = policy.get("compiler_driver_aliases")
    if not isinstance(aliases, dict) or any(
        language not in aliases
        or not isinstance(values, list)
        or not all(isinstance(value, str) for value in values)
        for language in policy["compiler_languages"]
        for values in [aliases.get(language)]
    ):
        raise spack.error.InstallError(f"Invalid compiler aliases in {path}")
    return policy


SANDBOX_POLICY = _load_sandbox_policy()
SANDBOX_COMMANDS = tuple(SANDBOX_POLICY["commands"])
...
HOST_RUNTIME_READ_PATHS = tuple(SANDBOX_POLICY["host_runtime_read_paths"])
FILE_RUNTIME_READ_PATHS = tuple(SANDBOX_POLICY["file_runtime_read_paths"])
COMPILER_LANGUAGES = tuple(SANDBOX_POLICY["compiler_languages"])
COMPILER_DRIVER_ALIASES = {
    language: tuple(aliases)
    for language, aliases in SANDBOX_POLICY["compiler_driver_aliases"].items()
}
COMPILER_PROGRAMS = tuple(SANDBOX_POLICY["compiler_programs"])
BINUTILS_PROGRAMS = tuple(SANDBOX_POLICY["binutils_programs"])
COREUTILS_INSTALL_PROGRAMS = tuple(SANDBOX_POLICY["coreutils_install_programs"])
COREUTILS_FILE_PROGRAMS = tuple(SANDBOX_POLICY["coreutils_file_programs"])
COREUTILS_UTIL_PROGRAMS = tuple(SANDBOX_POLICY["coreutils_util_programs"])
BUILD_UTILITIES_PROGRAMS = tuple(SANDBOX_POLICY["build_utilities_programs"])
SCRIPT_INTERPRETER_PROGRAMS = tuple(SANDBOX_POLICY["script_interpreter_programs"])
BUILD_PROGRAMS = (
    COMPILER_PROGRAMS
    + BINUTILS_PROGRAMS
    + COREUTILS_INSTALL_PROGRAMS
...
COMPILER_FILES = tuple(SANDBOX_POLICY["compiler_files"])
...
def worker_function(
    ...
    except spack.util.sandbox.JsonWorkerError as e:
        print(e, file=sys.stderr)
        exit_code = ExitCode.BUILD_ERROR
