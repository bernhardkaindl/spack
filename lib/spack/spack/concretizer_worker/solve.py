...
def _worker_paths(context: spack.context.SpackContext) -> Tuple[List[str], List[str]]:
    """Derive worker filesystem capabilities from the trusted parent context."""
    from spack.solver.asp import ConcretizationCache

    misc_cache = pathlib.Path(spack.caches.misc_cache_location(config=context.config))
    cache_root = context.config.get("concretizer:concretization_cache:url", None)
    if cache_root is None:
        cache_root = os.path.join(str(misc_cache), "concretization")
    concretization_cache = ConcretizationCache(
        root=spack.config.canonicalize_path(cache_root, config=context.config),
        entry_limit=context.config.get("concretizer:concretization_cache:entry_limit", 1000),
    ).root
    ...
    for scope in context.config.active_scopes:
    ...
    try:
        with use_buildcache_snapshot(validated.buildcache_specs), use_host_libc_snapshot(
            ...
            with warnings.catch_warnings(record=True) as caught:
                solver = Solver(context=spack.context.default(), specs_factory=specs_factory)
...
def solve_in_worker(
    ...
    *,
    ...
    buildcache_specs = (
        ...
        if buildcache_reuse_enabled(context.config)
        else []
    )
    read_roots, write_roots = _worker_paths(context)
    ...
    max_response_bytes = context.config.get(
