...
class SeccompSandbox:
    ...
    def _deny_non_unix_sockets(self, context) -> None:
        """Allow local Unix sockets while denying addressable network sockets."""
        syscall = self._get_syscall_number("socket")
        if syscall >= 0:
            self._rule_add_not_equal(
                context, syscall, 0, LINUX_AF_UNIX, SECCOMP_RET_ERRNO | errno.EPERM
            )
    ...
    def apply(
        ...
        try:
            ...
            if block_sockets:
                self._deny_non_unix_socketpairs(context)
                self._deny_non_unix_sockets(context)
            self._load(context)
        finally:
            self.libseccomp.seccomp_release(context)
