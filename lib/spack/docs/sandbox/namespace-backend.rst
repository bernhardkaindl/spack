...
The existing Landlock and seccomp stack is additive to this backend, not a
replacement. Landlock continues to deny writes and restricts read and execute
access to selected roots. Seccomp continues to deny sockets, process creation,
and IPC. The namespace backend adds the ability to truly hide host filesystem
content rather than only denying access to it.
...
4. The setup child executes the worker binary as a new process inside the
   ...
   descriptors.
5. The worker applies Landlock and seccomp inside the namespaces before
   importing any recipe code.
...
Relationship to Landlock and seccomp
------------------------------------

The namespace backend does not replace Landlock or seccomp. It complements
them:
...
* Seccomp denies sockets, process creation, and IPC inside the namespace.
  The network supervisor and invocation proxy continue to mediate worker TCP
  connections.
* The combination means the worker sees only the content the trusted parent
  selected, cannot write to paths it should not write to, cannot create
  sockets or child processes, and cannot use blocked IPC.

The namespace backend therefore provides defense in depth: even if a Landlock
rule were incorrectly broad, the mount tree limits what the worker can see,
and seccomp limits what it can do with what it sees.
...
and then applies Landlock and seccomp inside the namespace.

The existing Landlock and seccomp policies remain valid inside the namespace.
...
worker is Landlock and seccomp, which are already implemented.
...
* [ ] Landlock and seccomp remain in force inside the namespace.
