...
Its setup recreates inherited lock and store state before confinement so recipe patch methods can query installed dependency prefixes safely. When available, the worker runs inside a Linux user and mount namespace as described in :doc:`namespace-backend`.
